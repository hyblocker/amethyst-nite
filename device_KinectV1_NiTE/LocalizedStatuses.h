...
namespace nite
{
	...
	inline std::map<std::wstring, std::wstring> status_no_device_map
	{
		{
			L"en-US",
			L"NO DEVICES\nR_E_NO_DEVICE\nThere were no proper OpenNI devices (drivers) to initialize with. Restarting Amethyst may help."
		},
		{
			L"lc-LC",
			L"U BROKE DAMM\nR_E_NO_DEVICE\nDer wr naw nowmal OPNII thingies to shtart twackingw wif. Rwstwawew maay hewlp."
		}
	};

	inline std::map<std::wstring, std::wstring> status_openni_fail_map
	{
		{
			L"en-US",
			L"OPENNI FAIL\nR_E_OPENNI_FAIL\nOpenNI has failed to start up. Please check its resource files."
		},
		{
			L"lc-LC",
			L"OPENNII FAWW\nR_E_OPENNI_FAIL\nOPNII angwy n poutin at u! It thinkies ur a bashtard, get rekt."
		}
	};

	inline std::map<std::wstring, std::wstring> status_nite_fail_map
	{
		{
			L"en-US",
			L"NITE FAIL\nR_E_NITE_FAIL\nNiTE has failed to start up. Please check its resource files."
		},
		{
			L"lc-LC",
			L"NIITE FAIW\nR_E_NITE_FAIL\nNIITE angwy n poutin at u! It thinkies ur a bashtard, get rekt."
		}
	};
	...
	inline std::map<std::wstring, std::wstring> status_not_started_map
	{
		{
			L"en-US",
			L"NOT STARTED\nR_S_NOT_STARTED\nPlease click 'Refresh' to start the device handler."
		},
		{
			L"lc-LC",
			L"LAYIN DOWN\nR_S_NOT_STARTED\nDa d'vice lazy n lyin waitin f u to invit it to a pawwty."
		}
	};
}
